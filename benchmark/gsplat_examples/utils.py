...
def plain_ssim(img1: Tensor, img2: Tensor, padding: str = "valid") -> Tensor:
    """Standard SSIM (11x11 Gaussian, sigma=1.5, data range 1) as a drop-in for
    fused_ssim when the CUDA extension is unavailable. Same math, plain torch.

    Args:
        img1, img2: [B, C, H, W] in [0, 1].
        padding: "valid" (no padding, matches fused_ssim's default here) or "same".
    """
    c1, c2 = 0.01**2, 0.03**2
    coords = torch.arange(11, dtype=img1.dtype, device=img1.device) - 5
    g = torch.exp(-(coords**2) / (2 * 1.5**2))
    g = (g / g.sum()).reshape(1, 1, 1, 11)
    kernel = (g.transpose(2, 3) @ g).expand(img1.shape[1], 1, 11, 11)
    pad = 5 if padding == "same" else 0
    groups = img1.shape[1]

    def filt(x):
        return F.conv2d(x, kernel, padding=pad, groups=groups)
